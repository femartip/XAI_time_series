...
from PivotEval.SHAP_kernel import get_shap_rank_for_all_time_step
from SimplificationMethods.ORSalgorithm.Utils.line import interpolate_points_to_line
...
from simplifications import get_OS_simplification, get_RDP_simplification, get_VW_simplification, \
    get_bottom_up_simplification
...
def get_simplification(data, algo, alpha) -> List[SegmentedTS]:
    print(f"Getting Simplification {algo}...")
    if algo == "OS":
        simplifications = get_OS_simplification(time_series=data, alpha=alpha)
    elif algo == "RDP":
        simplifications = get_RDP_simplification(time_series=data, epsilon=alpha)
    elif algo == "VW":
        simplifications = get_VW_simplification(time_series=data, alpha=alpha)
    elif algo == "BU":
        simplifications = get_bottom_up_simplification(time_series=data, max_error=alpha, interpolate_segments=False)
    else:
        raise ValueError("Unknown algorithm")
    print(f"Got Simplification {algo}")
...
def get_pivot_points_and_simp_of_algo(algo, model_name, data, dataset_name, total_pivots):
    ...
    df = pd.read_csv(f"results/{dataset_name}/{model_name.split('.')[0]}_alpha_complexity_loyalty.csv")
    df = df[df["Type"] == algo]
    df = df[df["Num Segments"] >= total_pivots - 1]
    min_num_segs = min(df["Num Segments"])
    print(min_num_segs)
    df = df[df["Num Segments"] == min_num_segs]
    min_alpha_needed = sorted(df["Alpha"].tolist())[0]  #

    # Extract the simp corresponding to this alpha, run_simplification_algorithm
    simplifications = get_simplification(data, algo, min_alpha_needed)
    pivot_points = []
    full_ts = []
    for ts in simplifications:
        pivot_points.append(ts.x_pivots)
        full_ts.append(ts.line_version)

    return pivot_points, full_ts
...
def check_each(df, df_score_all, algo):
    ...
    error = 10 ** -4
    df[f"{algo}SimpHasXY"] = df.apply(
        lambda row: abs(row[f"{algo}Simp"][row["ShapIndex"]] - row[f"TS"][row["ShapIndex"]]) < error, axis=1)  #
    ...
    df[f"{algo}IncludesMaxShap"] = df.apply(lambda row: row[f"{algo}IncludesMaxShapIndex"] and row[f"{algo}SimpHasXY"],
                                            axis=1)
    ...
    df_score_all[f"{algo}Random"] = df.apply(lambda row: len(row[f"{algo}PivotPoints"]) / len(row["TS"]),
                                             axis=1).mean() * 100
    df_score_all[f"{algo}PeakRank"] = df.apply(
        lambda row: find_peak_rank(simplification=row[f"{algo}Simp"], full_ts=row["TS"],
                                   rank_of_time_steps=row[f"ShapRank"]), axis=1).mean()
    df_score_all[f"{algo}ExpectedPeakRank"] = df.apply(
        lambda row: (len(row["TS"]) + 1) / (len(row[f"{algo}PivotPoints"]) + 1), axis=1).mean()
    ...
    df_score_all[f"{algo}DistToMaxShap"] = df.apply(
        lambda row: abs(row[f"{algo}Simp"][row["ShapIndex"]] - row[f"TS"][row["ShapIndex"]]), axis=1).mean()
...
def check_how_often_most_important_time_step_is_pivot(model_name, dataset_name, total_pivots, recompute=False):
    ...
    if recompute or not os.path.exists(f"{folder_shap}/{file_shap}") or not os.path.exists(
            f"{folder_shap}/{file_shap_rank}"):
        max_shap_idx = []
        all_shap_ranks = []
        ...
        all_ranks = get_shap_rank_for_all_time_step(model_name=model_name, dataset_name=dataset_name,
                                                    background=org_dataset, instances_to_be_explained=org_dataset)
    ...
    for algo in our_algos:
        pivots, simp = get_pivot_points_and_simp_of_algo(algo=algo, model_name=model_name, data=org_dataset,
                                                         dataset_name=dataset_name, total_pivots=total_pivots)
    ...
    for algo in all_algos:
        print(f"{algo} MAX SHAP is in pivots:", f"{df_score_all[f'{algo}Score']}%",
              f"({round(df_score_all[f'{algo}Random'], 2)}%)")

    for algo in all_algos:
        print(f"{algo} Average Rank Of Most Influential Pivot: ", f"{df_score_all[f'{algo}PeakRank']}",
              f"({round(df_score_all[f'{algo}ExpectedPeakRank'], 2)})")
...
def aggregate_over_all_dfs(list_of_dfs: List[pd.DataFrame]) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    for dataset_name in dataset_names:
        ...
        if redo:
            ...
            df = check_how_often_most_important_time_step_is_pivot(dataset_name=dataset_name, model_name=model_name,
                                                                   recompute=recompute, total_pivots=curr_pivots)

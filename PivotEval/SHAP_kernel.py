...
def get_shap_rank_for_all_time_step(model_name, dataset_name, background, instances_to_be_explained):
    ...
    shap_class_curr = [shaps if c == 0 else -shaps for shaps, c in
                       zip(shap_values, c)]  # Shape: (24,) - importance for class 0
...
if __name__ == "__main__":
    ...
    instance_to_be_explained = dataset[
        idx_to_be_explained]  # Randomly select up to 100 unique elements without replacement
    ...
    shap_values = get_shap_values(model_name=model_name, dataset=selected_dataset, dataset_name=dataset_name,
                                  instance_to_be_explained=instance_to_be_explained)
    print(shap_values.shape)

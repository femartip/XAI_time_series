...
from SimplificationMethods.BottumUp.bottomUpLSF import get_swab_approx as get_swab_lsf_approx
...
def get_bottom_up_lsf_simplification(time_series: np.ndarray, max_error: float, interpolate_segments: bool) -> list[SegmentedTS]:
    """
    Apply Bottom Up LSF algorithm to simplify all time series in the dataset.
    """
    ts_simplifications = []
    first = False
    max_error = max_error
    ...
    for ts_y in time_series:
        ts_x = list(range(len(ts_y)))
        simplification = get_swab_lsf_approx(ts_y, max_error=max_error)
        simp_x = simplification.x_pivots
        simp_y = simplification.y_pivots
        num_segments = simplification.num_real_segments

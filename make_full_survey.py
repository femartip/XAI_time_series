...
from reportlab.platypus import Frame, FrameBreak, PageTemplate, BaseDocTemplate, Paragraph, Table, TableStyle
...
def build_pdf_one_dataset(prot_images: Dict[int, list[io.BytesIO]], test_images: Dict[str, list[io.BytesIO]],
    ...
    if len(prot_images) == 3:
        print("UCM")
    prototype_pdf = []
    prototype_pdf.append(Paragraph(f"Configuration: <b>{config_for_student}</b>", BODY_TEXT_STYLE))
    ...
    for i, label in enumerate(prot_images.keys()):
        ...
        colour = colour_names[i]
        # prototype_pdf.append(Paragraph(f"<font color='{colour}'>{colour}</font>", BODY_TEXT_STYLE))
        for j, img in enumerate(one_class_images):
            if len(prot_images.keys()) == 3 and i == 1 and j == 2:
                prototype_pdf.append(Spacer(1, 1 * inch))
            buf_img = Image(img, width=3.5 * inch, height=1.3 * inch)
            prototype_pdf.append(buf_img)

            data = [
                [ClassSwatch(c, getattr(colors, "white")) if c != colour else ClassSwatch(c, getattr(colors, colour))
                 for c in colour_names]]
            table = Table(data, colWidths=[60] * len(colour_names))
            table.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
            prototype_pdf.append(table)

        if len(prot_images.keys()) == 2 and i == 0:
            prototype_pdf.append(FrameBreak())
            prototype_pdf.append(Spacer(1, 1 * BODY_TEXT_STYLE.leading))
            prototype_pdf.append(Spacer(1, 1 * BODY_TEXT_STYLE.leading))

    test_pdf = []
    test_pdf.append(Paragraph(f"Configuration: <b>{config_for_student}</b>", BODY_TEXT_STYLE))

    # test_pdf.append(Paragraph("Which class do you think this is (pink or blue).", BODY_TEXT_STYLE))
    test_pdf.append(
        Paragraph("<b>Test Samples</b> - Assign the colour you think corresponds to each of the following samples.",
                  BODY_TEXT_STYLE))
    ...
    full_pdf = prototype_pdf + [FrameBreak()] + test_pdf
    full_pdf.append(FrameBreak())
...
def convert_to_images(proto_types: Dict[str, np.ndarray], test_instances: np.ndarray) -> Tuple[
    Dict[str, List[io.BytesIO]], List[io.BytesIO]]:
...
def make_pdf_for_dataset_loyalty(dataset, loyalty_level, config_for_student, group, student_number):
    full_train_instances_simplified, full_test_instances_simplified = get_train_and_test_instances(dataset,
                                                                                                   loyalty_level)
...
def make_pdf_for_current_student(group, student_number):
    ...
    datasets_and_loyalty_for_student = [(*get_dataset_and_loyalty_from_config(config), config) for config in
                                        config_for_student]

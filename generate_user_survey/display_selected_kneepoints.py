...
from generate_user_survey.configurations import selected_datasets_to_be_in_survey, loyalty_value_for_each_dataset
...
def visualise_the_selected_loyalty_values(dataset_name, model_name, selected_kneepoints):
    ...
    # At most length-1 segments
    ax.scatter(length - 1, 100, marker="x", label=f"Full TS:Loy:{100}%, Alpha:N/A, Seg:{length - 1}")
    ax.set_title(f"{dataset_name}")
    ax.legend(loc="lower right")
    folder_path = "generate_user_survey/paper_figs"
    os.makedirs(folder_path, exist_ok=True)
    fig.savefig(f"{folder_path}/{dataset_name}_loyalty_values.png")
...
if __name__ == "__main__":
    ...
    for dataset_name in datasets:
        ...
        visualise_the_selected_loyalty_values(dataset_name, model_name, selected_kneepoints)
